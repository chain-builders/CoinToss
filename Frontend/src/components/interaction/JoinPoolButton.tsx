...
const PoolsInterface: React.FC = () => {
  ...
  const closeModal = (poolId:number, entryFee:BigInt) => {
   
    setIsModalOpen(false);
    // handleJoinPool(poolId,entryFee)
      ...
      <AnimatePresence>
        {isModalOpen && selectedPool && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={closeModal(selectedPool.id, selectedPool.entryFee)} // Close modal when clicking outside
          >
            <motion.div
